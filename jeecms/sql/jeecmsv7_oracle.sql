...
alter table JC_CONTENT_EXT modify (DESCRIPTION VARCHAR2(2048 Char));

alter table JC_CONTENT_ATTACHMENT modify (ATTACHMENT_NAME VARCHAR2(256 Char));

alter table JC_CONTENT_ATTACHMENT modify (FILENAME VARCHAR2(256 Char));

alter table JC_USER add constraint username_unique unique(USERNAME);

alter table JC_CONTENT_EXT modify Author NOT NULL;
alter table JC_CONTENT_EXT modify Origin NOT NULL;

CREATE TABLE JC_CONTENT_PRODUCT 
(
  CONTENT_ID      NUMBER(11) NOT NULL,
  PRODUCT_CODES   VARCHAR2(128 CHAR),
  CONSTRAINT PK_JC_CONTENT_PRODUCT PRIMARY KEY (CONTENT_ID)
);

COMMENT ON TABLE JC_CONTENT_PRODUCT IS '文章相关基金表';

COMMENT ON COLUMN JC_CONTENT_PRODUCT.CONTENT_ID IS '文章ID';

COMMENT ON COLUMN JC_CONTENT_PRODUCT.PRODUCT_CODES IS '相关基金';

ALTER TABLE JC_CONTENT_PRODUCT ADD CONSTRAINT FK_JC_PRODUCT_CONTENT FOREIGN KEY (CONTENT_ID) REFERENCES JC_CONTENT (CONTENT_ID);
